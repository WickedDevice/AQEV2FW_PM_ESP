...
#define EEPROM_PM_SENSITIVITY     (EEPROM_MQTT_PASSWORD - 4)      // float value, 4-bytes, the sensitivity from the sticker  [UNUSED]
#define EEPROM_PM_CAL_SLOPE       (EEPROM_PM_SENSITIVITY - 4)     // float value, 4-bytes, the slope applied to the sensor   [UNUSED] 
#define EEPROM_PM_CAL_OFFSET      (EEPROM_PM_CAL_SLOPE - 4)       // float value, 4-btyes, the offset applied to the sensor
#define EEPROM_CO_SENSITIVITY     (EEPROM_PM_CAL_OFFSET - 4)      // float value, 4-bytes, the sensitivity from the sticker  [UNUSED]
...
#define EEPROM_PM_BASELINE_VOLTAGE_TABLE (EEPROM_NTP_TZ_OFFSET_HRS - 4) // array of (up to) six structures for baseline offset characterization over temperature
//  /\
...
#define EEPROM_BACKUP_CO_SENSITIVITY     (EEPROM_BACKUP_PM_CAL_OFFSET + 4)
#define EEPROM_BACKUP_PM_CAL_OFFSET      (EEPROM_BACKUP_PM_CAL_SLOPE + 4)
#define EEPROM_BACKUP_PM_CAL_SLOPE       (EEPROM_BACKUP_PM_SENSITIVITY + 4)
#define EEPROM_BACKUP_PM_SENSITIVITY    (EEPROM_BACKUP_MQTT_PASSWORD + 32)
...
#define BACKUP_STATUS_PM_CALIBRATION_BIT          (5)
...
const char cmd_string_pm_off[] PROGMEM      = "pm_off     ";
...
void restore(char * arg) {
  ...
  if (strncmp(arg, "defaults", 8) == 0) {
    ...
    eeprom_write_block("aqev2_pm_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
  }  
  else if (strncmp("key", arg, 3) == 0) {
    ...
    if (!BIT_IS_CLEARED(backup_check, BACKUP_STATUS_PM_CALIBRATION_BIT)) {
      Serial.println(F("Error: PM calibration must be backed up  "));
      Serial.println(F("       prior to executing a 'restore'."));
      return;
    }
...
boolean add_baseline_voltage_characterization(char * arg, uint32_t eeprom_table_base_address){
  ...
  if(!empty_location_found){
    ...
    if(eeprom_table_base_address == EEPROM_PM_BASELINE_VOLTAGE_TABLE){
      Serial.print(F("pm"));
    }
...
void pm_baseline_voltage_characterization_command(char * arg){
  baseline_voltage_characterization_command(arg, EEPROM_PM_BASELINE_VOLTAGE_TABLE);
}
...
boolean publishHeartbeat(){
  ...
  "\"converted-value\":%d,"
  "\"firmware-version\":\"%s\","
  "\"publishes\":[\"pm\",\"temperature\",\"humidity\"],"
  "\"counter\":%lu"
...
void pm_convert_from_volts_to_micrograms_per_cubic_meter(float volts, float * converted_value, float * temperature_compensated_value){
  static boolean first_access = true;
